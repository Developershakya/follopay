...
<head>
    ...
    <title>Verify OTP - <?php echo SITE_NAME ?? 'EarnApp'; ?></title>
...
<body class="flex items-center justify-center min-h-screen p-4">
    <div class="bg-white rounded-2xl shadow-2xl w-full max-w-md p-8">
        <div class="text-center mb-8">
            ...
            <h1 class="text-2xl font-bold text-gray-800">Verify Reset Code</h1>
            <p class="text-gray-600 mt-2">Enter the 6-digit code sent to your email</p>
        </div>

        <form id="verifyResetForm" class="space-y-6">
            <div>
                <label class="block text-sm font-medium text-gray-700 mb-3">Reset Code</label>
        ...
        <div class="mt-6 text-center pt-6 border-t">
            <a href="?page=forgot-password" class="text-gray-600 hover:text-gray-800 text-sm">
    ...
    <script>
        ...
        otpInputs.forEach((input, index) => {
            input.addEventListener('keyup', function(e) {
                ...
                if (otp.length === 6) {
                    document.getElementById('verifyResetForm').dispatchEvent(new Event('submit'));
                }
            });
        ...
        // Form submission
        document.getElementById('verifyResetForm').addEventListener('submit', async function(e) {
            e.preventDefault();
            ...
            formData.append('action', 'verify-reset-otp');
            ...
            try {
                ...
                if (data.success) {
                    showMessage('Code verified! Setting new password...', 'success');
                    setTimeout(() => {
                        window.location.href = '?page=reset-password-form';
                    }, 1500);
                } else {
        ...
        function updateResendButton() {
            ...
            const resendBtn = document.querySelector('button[onclick="resendResetOTP()"]');
        ...
        async function resendResetOTP() {
            ...
            // Get email from session if available, or show alert
            formData.append('email', ''); // This needs to be handled on backend
